...
void initialize(Simulation *sim) {
  ...
  sim->spice_lvl = (int **)malloc(sizeof(int *)*sim->height);
  ...
  for (int i=0; i<HEIGHT; i++) {
    ...
    sim->spice_lvl[i] = (int *)malloc(sizeof(int)*sim->width);
    sim->spice_cap[i] = (int *)malloc(sizeof(int)*sim->width);
  ...
  for (int i=0; i<sim->height; i++) {
    for (int j=0; j<sim->width; j++) {
      ...
      sim->spice_lvl[i][j] = SUGAR_CAP_SETTING4[i][sim->width-j-1];
      sim->spice_cap[i][j] = SUGAR_CAP_SETTING4[i][sim->width-j-1];
    }
  }
  ...
  for (int i=0; i<INIT_AGENTS; i++) {
    ...
    initialize_agent(sim, agent, -1, -1, -1, -1, -1, -1, -1);
  }
}
